...
function Widget({ trendingResult, followResult }) {
  return (
    <div className="ml-8 hidden space-y-5 py-1 lg:inline xl:w-[450px]">
      <div className="sticky top-0 z-50 w-11/12 bg-black py-1.5 xl:w-9/12">
        <div className="relative flex items-center rounded-full bg-[#202327] p-3">
          ...
          <input
            type="text"
            className="absolute inset-0 h-full w-full rounded-full border border-transparent bg-transparent pl-11 text-[#d9d9d9] placeholder-gray-500 outline-none focus:border-[#1d9bf0] focus:bg-black focus:shadow-lg"
            placeholder="Search Twitter"
          />
        </div>
      </div>

      <div className="w-11/12 space-y-3 rounded-xl bg-[#15181c] pt-2 text-secondary xl:w-9/12">
        <h4 className="px-4 text-xl font-bold">What's happenning</h4>
        {trendingResult.map((trending, index) => (
          <Trending key={index} result={trending} />
        ))}
